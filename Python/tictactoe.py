...
-1 if the board is not yet finished AND no one has won
    yet (there are empty spots),
1 if "X" won,
2 if "O" won,
0 if it's a cat's game (i.e. a draw).

You may assume that the board passed in is valid in
the context of a game of Tic-Tac-Toe.
"""
...
def is_solved(board: List[List[int]]) -> int:
    def diagonal(n: int) -> bool:
        if all(np.array([board[i, i] for i in range(len(board))]) == [n] * len(board)):
            return True

        if all(np.array([board[::-1][i, i] for i in range(len(board))]) == [n] * len(board)):
            return True
    ...
    if diagonal(n := 1) or diagonal(n := 2):
        return n

    if 0 in np.array(board, ndmin=1):
        return -1
