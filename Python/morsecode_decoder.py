...
letters are used. When the message is written in Morse code,
a single space is used to separate the character codes and 3
spaces are used to separate words. For example, the message
...
def decode_morse(morse_code: str) -> str:
    return "".join(MORSE_CODE.get(i, " ") for i in re.findall(r'\S+|\s{3}', morse_code)).strip()
